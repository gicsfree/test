...
int main(int argc, char** argv)
{
    ...
    if (0 == strncmp(is_exist, exist, strlen(exist)))
    {
        ...
        {
            fclose(fp);
            cSend = Send(cClient, update, sizeof(update), 0);
            if(cSend < 0)
                {
                    printf("send() failure!\n");
                    return -1;
                }
            if (0 == update_list(cClient, filename))
                printf("update successfully !\n");
        }
        else
        {
            cSend = Send(cClient, download, sizeof(download), 0);
            if(cSend < 0)
                {
                    printf("send() failure!\n");
                    return -1;
                }
            if (0 == download_list(cClient, filename))
